			...
			<div class="entry-dscr">
				...
				<div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
					<i class="fa fa-map-marker" aria-hidden="true"></i>
      				<span itemprop="postalCode">{{ $row->linked->postal_code }}</span>
      				<span itemprop="addressLocality">{{ $row->linked->locality }}</span>,
      				(<span itemprop="addressRegion">{{ $row->linked->administrative_area_level_2_short }}</span>)
      				<meta itemprop="addressCountry" content="{{ $row->linked->country_short}}" /> 
    			</div>
				--}}
				{{--
